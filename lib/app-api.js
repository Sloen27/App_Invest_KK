...
export async function handleApiRequest(req, res, url) {
  ...
  if (url.pathname === "/api/bonds/ofz") {
    await sendBondResponse(res, () => getBondsByKind("ofz", Object.fromEntries(url.searchParams)), true);
    return;
  }

  if (url.pathname === "/api/bonds/corporate") {
    await sendBondResponse(
      res,
      () => getBondsByKind("corporate", Object.fromEntries(url.searchParams)),
      true
    );
    return;
  }
  ...
  if (url.pathname === "/api/bonds/candidates") {
    await sendBondResponse(res, () => getBondCandidates(Object.fromEntries(url.searchParams)));
    return;
  }

  if (url.pathname === "/api/bonds/basket") {
    const body = req.method === "POST" ? await readJson(req) : Object.fromEntries(url.searchParams);
    await sendBondResponse(res, () => buildBondBasket(body));
    return;
  }
...
async function buildIncomeCalendar(accountId) {
  ...
  for (const position of positions) {
    ...
    const quantity = moneyValue(position.quantity);
...
async function appendCorporateEvents({ events, errors, path, request, rootKeys, label, quantity, kind }) {
  try {
    const response = await tInvest(path, request);
    const items = rootKeys.flatMap((key) => (Array.isArray(response[key]) ? response[key] : []));

    for (const item of items) {
      const amountPerOne = item.dividendNet || item.payOneBond || item.couponValue || item.value;
      events.push({
        kind,
        instrument: label,
        ...
        quantity,
        amountPerOne,
        amount: multiplyMoney(amountPerOne, quantity),
        source: item
      });
    }
  } catch (error) {
...
function multiplyMoney(value, quantity) {
  ...
  const total = amount * (Number(quantity) || 0);
  return {
    units: Math.trunc(total),
    nano: Math.round((total - Math.trunc(total)) * 1_000_000_000),
    currency: value?.currency || value?.currencyId || "rub"
  };
}
...
async function sendBondResponse(res, loader, shouldLimit = false) {
  try {
    const data = await loader();
    sendJson(res, 200, shouldLimit ? limitBondResponse(data) : data);
  } catch (error) {
    if (!isMoexUnavailableError(error)) throw error;

    sendJson(res, 503, {
      error: "MOEX ISS временно недоступен",
      code: "MOEX_UNAVAILABLE",
      hint:
        "Vercel Function не смогла подключиться к iss.moex.com. Попробуйте обновить позже; если ошибка повторяется, проверьте регион Vercel или задайте MOEX_ISS_BASE на доступный прокси/зеркало MOEX ISS.",
