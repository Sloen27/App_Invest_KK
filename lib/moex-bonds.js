  ...
  for (const item of items) {
    try {
      ...
      enriched.push({
        ...
        reminder: buildBuyReminder(item, schedule.nextCoupon),
...
function buildBuyReminder(item, nextCoupon) {
  if (!nextCoupon?.date) return null;

  const couponDate = startOfDay(new Date(nextCoupon.date));
  if (Number.isNaN(couponDate.getTime())) return null;

  const reminderDate = addDays(couponDate, 1);
  const windowEnd = addDays(couponDate, 7);
  const name = item.shortName || item.name || item.secid;

  return {
    date: formatIsoDate(reminderDate),
    windowEnd: formatIsoDate(windowEnd),
    title: `Проверить облигацию ${name} после купона`,
    note: `Купон ${nextCoupon.date} уже должен пройти. Проверьте цену, НКД, ликвидность и оферты перед решением.`
  };
}
