...
- Позволяет сохранить напоминания по облигациям в локальном браузере и скачать `.ics` для Apple Calendar.
...
Напоминания по облигациям хранятся только в `localStorage` текущего браузера. Кнопка `Apple Calendar` скачивает `.ics`-файл: откройте его на Mac или iPhone и выберите календарь, куда добавить событие. Сервер не получает доступ к вашему Apple Calendar.
